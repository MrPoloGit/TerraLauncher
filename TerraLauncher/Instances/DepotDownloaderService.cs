...
public static class DepotDownloaderService {
	...
	private static string ToolDir => Path.Combine(InstanceManager.ToolsRoot, "DepotDownloader");
	private static string BinaryPath => Path.Combine(ToolDir,
		OperatingSystem.IsWindows() ? "DepotDownloader.exe" : "DepotDownloader");
	...
	public static async Task<bool> EnsureInstalledAsync(DownloadProgressWindow ui, CancellationToken ct) {
		...
		if (assetUrl == null) {
			ui.AppendLog($"No DepotDownloader build found for {os}-{arch}.");
			return false;
		}
		...
		if (!OperatingSystem.IsWindows() && File.Exists(BinaryPath)) {
			File.SetUnixFileMode(BinaryPath, File.GetUnixFileMode(BinaryPath)
	...
	public static async Task<string?> DownloadTerrariaAsync(DownloadProgressWindow ui, VersionEntry entry,
		...
		long depot = OperatingSystem.IsWindows() ? entry.DepotIds?.Windows ?? 105601
			: OperatingSystem.IsMacOS() ? entry.DepotIds?.Mac ?? 105602
			: entry.DepotIds?.Linux ?? 105603;

		string? manifest = OperatingSystem.IsWindows() ? entry.ManifestIds?.Windows
			: OperatingSystem.IsMacOS() ? entry.ManifestIds?.Mac
			: entry.ManifestIds?.Linux;
		bool hasManifest = !string.IsNullOrEmpty(manifest) && !manifest.StartsWith("TODO");

		if (!hasManifest && entry.Version != "latest") {
			string osName = OperatingSystem.IsWindows() ? "Windows"
				: OperatingSystem.IsMacOS() ? "macOS" : "Linux";
			ui.AppendLog($"No Steam manifest ID is known for Terraria {entry.Version} on {osName}.");
		...
		ui.AppendLog($"Running DepotDownloader (app {TerrariaAppId}, depot {depot}"
			+ (hasManifest ? $", manifest {manifest})" : ", latest manifest)"));
		...
		var stderrTask = PumpLinesAsync(proc.StandardError, ui);
		...
		if (!OperatingSystem.IsWindows())
			Downloader.MarkExecutables(installDir);

		string? exe = Downloader.FindExecutable(installDir, InstanceCategory.Terraria);
		if (exe == null)
	...
	private static async Task PumpStdoutAsync(Process proc, DownloadProgressWindow ui, CancellationToken ct) {
		...
		while ((read = await reader.ReadAsync(buffer, 0, buffer.Length)) > 0) {
			...
			if (IsAuthPrompt(tail)) {
				ui.AppendLog(tail);
				pending.Clear();
				string? code = await Dispatcher.UIThread.InvokeAsync(() =>
					TextPromptWindow.ShowAsync(ui, "Steam Guard",
						tail.Trim(), "Code"));
	...
	private static bool IsAuthPrompt(string tail) {
		...
		return tail.Contains("auth code", StringComparison.OrdinalIgnoreCase)
			|| tail.Contains("authentication code", StringComparison.OrdinalIgnoreCase)
			|| tail.Contains("two-factor", StringComparison.OrdinalIgnoreCase)
			|| tail.Contains("2 factor", StringComparison.OrdinalIgnoreCase)
			|| tail.Contains("Steam Guard", StringComparison.OrdinalIgnoreCase);
	}
}
